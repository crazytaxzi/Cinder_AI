...
const schema = z.object({
  ...
  CINDER_VOICE_SOCIAL_MODEL: z.string().default('gpt-5.4-mini'),
  ...
  CINDER_VOICE_CONTEXT_EVENT_LIMIT: z.coerce.number().int().min(2).max(30).default(6),
  ...
  OPENAI_TTS_INSTRUCTIONS: z.string().default('A small mischievous imp with a lightly gravelly, raspy texture: smug, playful, crisp, and emotionally expressive. Infer the emotional subtext of each line and vary timing, emphasis, energy, and pauses naturally. Let jokes sound amused, warnings serious, conspiratorial lines intimate, and excitement genuinely animated. Keep the rasp subtle so every word stays clear. Never sound corporate, flat, or monotone.'),
  CINDER_VOICE_CLOUD_TTS: z.enum(['true', 'false']).default('true').transform((value) => value === 'true'),
  CINDER_VOICE_CLOUD_TTS_USD_PER_MINUTE: z.coerce.number().min(0).max(1).default(0.015),
  CINDER_VOICE_SPEED: z.coerce.number().min(0.25).max(2).default(0.468),
  CINDER_VOICE_PITCH: z.coerce.number().min(0.25).max(2).default(0.538055352),
